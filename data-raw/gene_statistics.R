...
gene_statistics <-
  read_tsv(
    "../../projects/2015_Bactrap_Chat_EAE_acute_v2/deseq2/results/statResult_Eip_vs_Ein.txt"
  ) |>
  rename(log2_foldchange = log2FoldChange) |>
  mutate(neg_log10_padj = -log10(padj)) |>
  mutate(
    is_cand = if_else(
      abs(log2_foldchange) > 1 & padj <= 0.05,
      TRUE,
      FALSE,
      FALSE
    ),
...
gene_statistics |>
  ...
  add_data_points(
    data = filter_rows(is_cand & direction == "up"),
    color = "red",
    alpha = 0.2
  ) |>
  add_data_points(
    data = filter_rows(is_cand & direction == "down"),
    color = "blue",
    alpha = 0.2
  ) |>
