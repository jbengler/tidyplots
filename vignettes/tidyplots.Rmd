...
df_demo %>% 
  tidy_plot(x = NA, y = value, fill = group) %>% 
  add_pie()
...
data_exprs %>% 
  ...
  plot_split(by = external_gene_name, ncol = 4, nrow = 4)
...
data_exprs %>% 
  ...
  add_error() %>% 
  add_jitter() %>% 
  plot_split(by = external_gene_name, ncol = 4, nrow = 4, widths = 10, heights = 10)
...
data_exprs %>% 
  ...
  add_error() %>% 
  add_jitter() %>% 
  plot_split(by = external_gene_name, ncol = 4, nrow = 4, widths = NA, heights = NA)
...
data_exprs %>% 
  ...
  add_jitter() %>% 
  plot_split(by = external_gene_name, ncol = 4, nrow = 4) %>%
  plot_save(filename = "test_multipage.pdf")
...
df_demo %>% 
  tidy_plot(x = category, y = value, color = category) %>% 
  add_mean() %>% 
  plot_save(filename = "test1.pdf") %>%
  add_error() %>% 
  plot_save(filename = "test2.pdf") %>% 
  add_jitter() %>% 
  plot_save(filename = "test3.pdf")
