...
- `color` and `fill` are always mapped to the same variable. However, you can still do one of the following: (1) reduce the  opacity of fill using the `alpha` or `fill_alpha` arguments, (2) set `color` or `fill` to a constant hex color within an `add_` function, or (3) set `color` or `fill` to `NA` within an `add_` function to to prevent it from being displayed.
...
p2 %>% adjust_labels(treatment, new_order = c("C")) %>% adjust_colors(new_colors, fill_alpha = 0.3)
p2 %>% adjust_colors(new_colors, fill_alpha = 0.3) %>% adjust_labels(treatment, new_order = c("C"))

p2 %>% adjust_colors(new_colors, fill_alpha = 0.3)
p2 %>% adjust_labels(treatment, new_names = new_names) %>% adjust_colors(new_colors, fill_alpha = 0.3)
p2 %>% adjust_colors(new_colors, fill_alpha = 0.3) %>% adjust_labels(treatment, new_names = new_names)
...
big_animals <- . %>% dplyr::filter(size > 300)

p3 <- 
  animals %>% 
  tidyplot(x = weight, y = size) %>% 
  add_scatter() %>% 
  add_scatter(subset_data = big_animals, color = "red") %>% 
  add_text(subset_data = big_animals, var = animal, color = "black")
...
study %>% 
  tidyplot(x = group, y = score, color = dose, dodge_width = 0.2) %>% 
  add_mean_bar(alpha = 0.3) %>% 
  add_mean_dash() %>% 
  add_mean_value()
...
spendings %>% 
  ...
  add_line(fun = max, linewidth = 0.75)
...
study %>% 
  ...
  add_mean_dash() %>% 
  add_error() %>% 
  add_scatter(subset_data = function(x) dplyr::filter(x, treatment == "C"))

study %>% 
  tidyplot(x = treatment, y = score, color = treatment) %>% 
  add_mean_dash() %>% 
  add_error() %>% 
  add_scatter(subset_data = function(x) dplyr::slice_max(x, score, n = 1, by = treatment))
...
animals %>%
  tidyplot(x = family, y = weight) %>%
  add_mean_bar(alpha = 0.3) %>%
  add_mean_value(angle = -90, vjust = -0.5, hjust = 0.5) %>%
  adjust_flip()
