...
df_demo %>% 
  tidyplot(x = group2, y = value, color = group, fill = group) %>% 
  add_mean_bar(alpha = 0.3) %>% 
  add_error() %>% 
  add_jitter()
...
df_demo %>% 
  ...
  add_violin() %>% 
  add_jitter() %>% 
  adjust_labels(category, labels = my_labels)
...
df_demo %>% 
  ...
  add_jitter() %>% 
  adjust_labels(category, labels = my_labels) %>% 
  adjust_color_palette(colors = my_colors, fill_alpha = 0.3) %>% 
  style_rotate_labels()
...
df_demo %>% 
  ...
  add_jitter() %>% 
  adjust_labels(category, labels = my_labels) %>% 
  adjust_color_palette(colors = my_colors[c(4,2)], fill_alpha = 0.3) %>% 
  style_rotate_labels()
...
df_demo %>% 
  ...
  add_error() %>% 
  add_jitter() %>% 
  adjust_order(category, reverse = TRUE)
...
df_demo %>% 
  ...
  add_error() %>% 
  add_jitter() %>% 
  adjust_order(category, sort_by = value)
...
df_demo %>% 
  ...
  add_error() %>% 
  add_jitter() %>% 
  adjust_order(category, levels = c("E", "M", "O", "D"))
...
df_demo %>% 
  tidyplot(x = category, y = value, color = group3, group = NA) %>% 
  add_line()
...
df_demo %>% 
  tidyplot(x = category, y = value, color = group3, fill = group3, group = NA) %>% 
  add_sem_ribbon() %>% 
  add_line() %>% 
  add_jitter()
...
df_demo %>% 
  tidyplot(x = group2, y = value, color = group, group = group) %>% 
  add_line() %>% 
  add_mean() %>% 
  add_error() %>% 
  add_jitter()
...
df_demo %>% 
  tidyplot(x = group2, y = value, fill = group) %>% 
...
df_demo %>% 
  ...
  add_error() %>% 
  add_jitter() %>% 
  adjust_description(title = "New title")

## split_plot
# tidyplot

data_exprs %>% 
  tidyplot(x = condition, y = expression, color = sample_type, fill = sample_type) %>% 
  add_mean() %>% 
  add_error() %>% 
  add_jitter() %>% 
