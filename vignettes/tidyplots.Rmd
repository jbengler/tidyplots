...
study %>% 
  ...
  add_mean_bar(alpha = 0.3) %>% 
  add_error_bar() %>% 
  adjust_description(title = "This is my fantastic plot", x_axis_title = "Treatment group", y_axis_title = "Disease score", legend_title = "")
```
...
study %>% 
  ...
  add_mean_bar(alpha = 0.3) %>% 
  add_error_bar() %>% 
  adjust_description(title = "$H[2]*O$", x_axis_title = "$E==m*c^{2}$", y_axis_title = "$TNF*alpha~level$", legend_title = "")
```
...
Themes are a great way to modify the look an feel of you plot without changing the representation of the data. You can stay with the defult tidyplots theme.
...
To learn more about these color schemes have a look at the article [Color schemes](https://jbengler.github.io/tidyplots/articles/Color-schemes.html).
