...
::: {.lead}
This getting started guide aims to empower individuals without a programming background to perform code-based plotting with tidyplots. We will begin by covering the necessary software tools in the prerequisites section. Next, we will discuss data preparation and describe the tidyplots workflow, which involves adding, removing, and adjusting plot components. We will then explore themes and multiplot layouts. Finally, we will conclude by offering useful resources to further enhance your code-based plotting skills.
:::
