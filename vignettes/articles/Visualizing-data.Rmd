...
Heatmaps are a great way to plot a _continuous variable_ across to two additional _variables_. To exemplify this, we will have a look at the `gene_expression` dataset.
...
Finally, while not strictly being a measure of central tendency you can also fit a curve through your data in order derive an abstracted representation of your data.
...
As you might appreciate, this dataset contains the `power` in gigawatt produced per `energy_source` in Germany between the `year` 2002 and 2023. Let's start with a `pie` plot.
...
Now we can appreciate that renewable power production had an overall bigger share than fossil power production in the time period between 2002 and 2023.
...
To test for differences between experimental groups, tidyplots offers the functions `add_stats_asterisks()` and `add_stats_pvalue()`. While the first one includes asterisks for symbolizing significance.
...
Note that I provided the `label` argument to the `add_data_labels()` function to chose the variable in the dataset that should be added as text labels.

Another thing to note is that there is quite some overlap of labels in the lower left of the plot. Let' try to repel the data labels from one another using the `add_data_labels_repel()` function.
...
There is lot tweaking that can be done with text labels. For more details have a look at the documentation of `add_data_labels_repel()`, the underlying function `ggrepel::geom_text_repel()` and [ggrepel examples](https://ggrepel.slowkow.com/articles/examples).
