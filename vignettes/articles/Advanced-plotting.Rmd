...
To remove the padding completely, you can use use the `remove_plot_area_padding()` function. But note that the extreme values now fall onto the axis and are partially cut off.
...
And here with the default `dodge_width = 0.8`. I added a reference line at day 10 to make it easier to appreciate the difference.
...
tidyplots is built on ggplot2, yet the two packages differ in several key aspects.

The most noticeable difference is probably that tidyplots consistently uses the pipe `%>%` to add plot components while ggplot uses `+` .
