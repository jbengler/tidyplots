#' @export
print.tidycolor <- function(x, max_colors = 42, return_html = FALSE, ...) {
  print_tidycolor(x, max_colors = max_colors, return_html = return_html)
}
#' @export
...
new_color_scheme <- function(
  x,
  name = "Untitled color scheme",
  reverse = FALSE
) {
  if (!is_hex_vector(x)) {
    cli::cli_abort("{.arg x} must be a vector of hex colors.")
  }
  if (reverse) {
    x <- rev(x)
  }
...
#' @md
#' @export
colors_discrete_friendly <- new_color_scheme(
...
#' @export
colors_discrete_seaside <- new_color_scheme(
  c("#8ecae6", "#219ebc", "#023047", "#ffb703", "#fb8500"),
  "colors_discrete_seaside"
)
#' @rdname colors_discrete_friendly
#' @export
colors_discrete_apple <- new_color_scheme(
  c(
    "#ff3b30",
    "#ff9500",
    "#ffcc00",
...
#' @rdname colors_discrete_friendly
#' @export
colors_discrete_friendly_long <- new_color_scheme(
  c(
    "#CC79A7",
    "#0072B2",
    "#56B4E9",
...
#' @rdname colors_discrete_friendly
#' @export
colors_discrete_okabeito <- new_color_scheme(
  unname(grDevices::palette.colors()[c(-1, -9)]),
  "colors_discrete_okabeito"
)
#' @rdname colors_discrete_friendly
#' @export
colors_discrete_ibm <- new_color_scheme(
  c("#5B8DFE", "#725DEE", "#DD227D", "#FE5F00", "#FFB109"),
  "colors_discrete_ibm"
)
#' @rdname colors_discrete_friendly
#' @export
colors_discrete_metro <- new_color_scheme(
  c("#4DACD6", "#4FAE62", "#F6C54D", "#E37D46", "#C02D45"),
  "colors_discrete_metro"
)
#' @rdname colors_discrete_friendly
#' @export
colors_discrete_candy <- new_color_scheme(
  c("#9b5de5", "#f15bb5", "#fee440", "#00bbf9", "#00f5d4"),
  "colors_discrete_candy"
)
#' @rdname colors_discrete_friendly
#' @export
colors_discrete_alger <- new_color_scheme(
  c("#000000", "#1A5B5B", "#ACC8BE", "#F4AB5C", "#D1422F"),
  "colors_discrete_alger"
)
#' @rdname colors_discrete_friendly
#' @export
colors_discrete_rainbow <- new_color_scheme(
  c(
    "#FF7777",
    "#FFAB74",
    "#FFE577",
...
#' @md
#' @export
colors_continuous_viridis <- new_color_scheme(
  viridisLite::viridis(265),
  name = "colors_continuous_viridis"
)
#' @rdname colors_continuous_viridis
#' @export
colors_continuous_magma <- new_color_scheme(
  viridisLite::magma(265),
  "colors_continuous_magma"
)
#' @rdname colors_continuous_viridis
#' @export
colors_continuous_inferno <- new_color_scheme(
  viridisLite::inferno(265),
  "colors_continuous_inferno"
)
#' @rdname colors_continuous_viridis
#' @export
colors_continuous_plasma <- new_color_scheme(
  viridisLite::plasma(265),
  "colors_continuous_plasma"
)
#' @rdname colors_continuous_viridis
#' @export
colors_continuous_cividis <- new_color_scheme(
  viridisLite::cividis(265),
  "colors_continuous_cividis"
)
#' @rdname colors_continuous_viridis
#' @export
colors_continuous_rocket <- new_color_scheme(
  viridisLite::rocket(265),
  "colors_continuous_rocket"
)
#' @rdname colors_continuous_viridis
#' @export
colors_continuous_mako <- new_color_scheme(
  viridisLite::mako(265),
  "colors_continuous_mako"
)
#' @rdname colors_continuous_viridis
#' @export
colors_continuous_turbo <- new_color_scheme(
  viridisLite::turbo(265),
  "colors_continuous_turbo"
)
#' @rdname colors_continuous_viridis
#' @export
colors_continuous_bluepinkyellow <- new_color_scheme(
  c(
    "#00034D",
    "#000F9F",
    "#001CEF",
...
#' @md
#' @export
colors_diverging_blue2red <- new_color_scheme(
  grDevices::colorRampPalette(c("#0000FF", "#FFFFFF", "#FF0000"))(17),
  "colors_diverging_blue2red"
)
#' @rdname colors_diverging_blue2red
#' @export
colors_diverging_blue2brown <- new_color_scheme(
  c(
    "#1961A5",
    "#2671B5",
    "#2D80BF",
...
#' @rdname colors_diverging_blue2red
#' @export
colors_diverging_BuRd <- new_color_scheme(
  RColorBrewer::brewer.pal("RdBu", n = 11),
  "colors_diverging_BuRd",
  reverse = TRUE
)
#' @rdname colors_diverging_blue2red
#' @export
colors_diverging_BuYlRd <- new_color_scheme(
  RColorBrewer::brewer.pal("RdYlBu", n = 11),
  "colors_diverging_BuYlRd",
  reverse = TRUE
)
#' @rdname colors_diverging_blue2red
...
generate_html <- function(x, max_colors) {
  ...
  color_bar <- color_vector <- paste0(
    "<span style='background-color:",
    x,
    "; display: inline-block; width:30px; height:20px;'></span>",
    collapse = ""
  )
  color_vector <- paste0(
    "<span style='background-color:",
  ...
  paste0(
    "<div style=\"margin-top: 60px; margin-bottom: 60px;\"><h4 style=\"font-family: 'Courier New', monospace;\">",
    name,
    "</h4><small style=\"font-family: 'Courier New', monospace;\">A tidyplots color scheme with ",
    size,
    " colors",
    downsample_text,
    ".</small><br>",
    color_bar,
    "<br><span style=\"font-family: 'Courier New', monospace;\">c(<br>",
    color_vector,
    ")</span></div>"
  )
}
